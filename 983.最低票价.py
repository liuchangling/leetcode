...
# 1 <= costs[i] <= 1000
# 
# 思路是 c[i]表示满足条件的第i天至少话多少钱，正向遍历思路较为清晰，逆序太绕了
# dp：

# 须出行 插入min(c[d-1] + costs[0], c[max(0,d-7)]+costs[1], c[max(0,d-30)] + costs[2])
# 无需出行 插入 c[d-1]
# 优化遍历次数：len(c) 到 days的长度即可
...
class Solution:
    def mincostTickets(self, days: List[int], costs: List[int]) -> int:
        ...
        for d in range(1,days[-1]+1) :
            if d in days:
                c.append(min(c[d-1] + costs[0], c[max(0,d-7)]+costs[1], c[max(0,d-30)] + costs[2]))
            else:
                c.append(c[d-1])
